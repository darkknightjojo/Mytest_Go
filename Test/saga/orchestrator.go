...
// execSubTx
// 根据给定编号执行对应短事务
func (c *ExecutionCoordinator) execSubTx(i int) {
	if c.aborted {
		return
	}
	start := time.Now()
	f := c.saga.subTxs[i].Func
...
func (c *ExecutionCoordinator) abort() {
	toCompensateLogs, err := c.logStore.GetSubTxLogsToCompensate(c.ExecutionID)
	...
	for i := 0; i < stepsToCompensate; i++ {
		...
		compensateFuncRaw := c.saga.subTxs[*toCompensateLog.StepNumber].CompensateFunc
		...
		if err := c.compensateTx(*toCompensateLog.StepNumber, params, compensateFuncValue); err != nil {
...
// compensateTx
// 需要回滚时，事务的赔偿操作
func (c *ExecutionCoordinator) compensateTx(i int, params []reflect.Value, compensateFunc reflect.Value) error {
	checkErr(c.logStore.AppendLog(&Log{
		ExecutionID: c.ExecutionID,
		Name:        c.saga.Name,
		Time:        time.Now(),
